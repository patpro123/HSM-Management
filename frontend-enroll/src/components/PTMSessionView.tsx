...
import PTMGroupNotifyModal from './PTMGroupNotifyModal';
...
export default function PTMSessionView({ sessionId, onBack, onSessionUpdate }: PTMSessionViewProps) {
  ...
  const [showGroupNotify, setShowGroupNotify] = useState(false);
  ...
  const handleStudentRemove = async (studentId: string) => {
    const appt = appointments.find(a => a.student_id === studentId);
    if (!appt) return;
    await apiDelete(`/api/ptm/appointments/${appt.id}`);
  ...
  const handleBulkTime = async () => {
    ...
    try {
      await Promise.all(
        visible.map(a => apiPut(`/api/ptm/appointments/${a.id}`, { scheduled_time: new Date(bulkTime).toISOString() }))
      );
      await loadSession();
      setBulkTime('');
    } finally {
  ...
  const handleNotifyAll = async () => {
    if (!window.confirm('Mark all scheduled appointments as "both notified"? WhatsApp messages still need to be sent individually.')) return;
    ...
    <div className="p-3 md:p-6 max-w-4xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            {editingTitle ? (
              <div className="flex items-center gap-2">
                <input
                  className="border border-slate-300 rounded-lg px-3 py-1.5 text-base font-bold focus:outline-none focus:ring-2 focus:ring-orange-400 flex-1 min-w-0"
                  value={titleDraft}
                  ...
                  autoFocus
                />
                <button onClick={handleSaveTitle} className="text-xs bg-orange-600 text-white px-3 py-1.5 rounded-lg whitespace-nowrap">Save</button>
                <button onClick={() => setEditingTitle(false)} className="text-xs text-slate-500">✕</button>
              </div>
            ) : (
              <h1
                className="text-lg font-bold text-slate-800 cursor-pointer hover:text-orange-600 flex items-center gap-1.5 flex-wrap"
            ...
            <p className="text-slate-500 text-xs mt-0.5">{formatDate(session.scheduled_date)}</p>
            {session.description && <p className="text-slate-600 text-sm mt-1">{session.description}</p>}
          </div>
          <select
            className="border border-slate-300 rounded-lg px-2 py-1.5 text-xs bg-white focus:outline-none flex-shrink-0"
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        <button
          ...
          className="bg-orange-600 text-white px-3 py-2 rounded-lg text-sm font-semibold hover:bg-orange-700 flex items-center gap-1.5"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          onClick={() => setShowGroupNotify(true)}
          disabled={total === 0}
          className="bg-green-600 text-white px-3 py-2 rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-40 flex items-center gap-1.5"
        >
          {WA_ICON}
        ...
        <button
          ...
          className="border border-slate-300 text-slate-600 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-slate-50 disabled:opacity-40 flex items-center gap-1.5"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
          <span className="hidden sm:inline">Mark All Notified</span>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <select
          className="flex-1 min-w-[120px] border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none"
          value={filterStatus}
      ...
      {/* Common time slot setter */}
      {appointments.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 p-3 bg-amber-50 rounded-lg border border-amber-100 mb-4">
          <span className="text-sm font-medium text-amber-800 whitespace-nowrap">Common slot:</span>
          <input
            type="datetime-local"
            className="flex-1 min-w-[180px] border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 bg-white"
            value={bulkTime}
          ...
          >
            {applyingBulkTime ? 'Applying...' : `Apply to ${visible.length}`}
          </button>
          {bulkTime && (
      ...
      {visible.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-slate-300">
          <p className="text-slate-500 font-medium text-sm">
            {total === 0 ? 'No students added yet' : 'No appointments match the filter'}
          </p>
          {total === 0 && (
            <button
              onClick={() => setShowAddStudents(true)}
              className="mt-3 bg-orange-600 text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-orange-700"
            >
              Add Students
            </button>
          )}
      ...
      {showGroupNotify && session && (
        <PTMGroupNotifyModal
          sessionTitle={session.title}
          sessionDate={session.scheduled_date}
          appointments={appointments}
          onClose={() => setShowGroupNotify(false)}
        />
      )}
    </div>
  );
